    ...
    return (
        <Container sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', maxWidth: "200px", maxHeight: "200x" }}>
            ...
            <Typography 
                sx={{ 
                    fontSize: '1.5em', 
                    color: props.color === 'white' ? 'primary.main' : 'white',
...
export default function Sentiment(colors) {
    ...
    return (
        <Container disableGutters sx={{ borderRadius: '10px', height: 'auto', marginBottom: 2}}>
            ...
            <Container sx={{
                ...
                {fearAndGreed?.data ? <GaugeChartComponent value={value} classification={classification} color={color} /> : <Loading />}
            </Container>
        </Container>
    );
}
