...
export default function Modal({ type, handleFormSubmit }) {
  ...
  return (
    <Dialog fullScreen={!!hide500} PaperProps={{ style: { borderRadius: '10px' } }} sx={{ margin: 0, padding: 0, backdropColor: 'background.default' }} open={loginIsOpen} onClose={handleToggleLoginModal}>
      <DialogTitle sx={{
        height: '50px', display: 'flex', justifyContent: 'space-between', backgroundColor: 'secondary.dark',
      }}
      >
      ...
      <DialogActions sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, backgroundColor: 'background.default' }}>
        <Box sx={{
          width: '100%', display: 'flex', justifyContent: { xs: 'space-around', md: 'end' }, height: '100%', alignContent: 'center',
        }}
        >
          {!forgotPassword && (
          <Button
          ...
          ><Typography sx={{ fontSize: '0.8em' }}>{ type === 'login' ? 'S\'inscrire' : 'J\'ai déjà un compte' }</Typography>
          </Button>
          )}
          {!forgotPassword && (
          <Button
            ...
            variant="contained"
          ><Typography sx={{ fontSize: '0.8em' }}>{ type === 'login' ? 'Se connecter' : 'S\'inscrire' }</Typography>
          </Button>
          )}
        </Box>
        {forgotPassword && (
